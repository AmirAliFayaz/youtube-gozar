...
logger = log.Logger("smlwb-log.txt")
logger.clear()
...
class DnsOverFragment:
    ...
    def query(self, server_name):
        ...
        if offline_ip is not None:
            if is_logging:
                logger.log('offline DNS -->', server_name, offline_ip)
            return offline_ip

        cache_ip = DNS_cache.get(server_name, None)
        if cache_ip is not None:
            if is_logging:
                logger.log('cached DNS -->', server_name, cache_ip)
            return cache_ip
        ...
        if is_logging:
            logger.log(f'online DNS Query', server_name)
        try:
            ...
            if ans.status_code == 200 and ans.headers.get('content-type') == 'application/dns-message':
                ...
                for x in answer_msg.answer:
                    if x.rdtype == dns.rdatatype.A:
                        ...
                        if is_logging:
                            logger.log("################# DNS Cache is : ####################")
                            logger.log(DNS_cache)
                            # later.
                            logger.log("#####################################################")
                        break
                if is_logging:
                    logger.log(f'online DNS --> Resolved {server_name} to {resolved_ip}')
                return resolved_ip
            else:
                if is_logging:
                    logger.log(f'Error: {ans.status_code} {ans.reason}')
...
class ThreadedServer(object):
    ...
    def handle_client_request(self, client_socket):
        ...
        if data[:7] == b'CONNECT':
            ...
            if is_logging:
                logger.log('redirect http to HTTPS', q_url)
            ...
            if is_logging:
                logger.log('Unknown Method', str(data[:10]))
        ...
        if is_logging:
            logger.log(server_name, '-->', server_port)
...
server = ThreadedServer('', listen_PORT)


def runApp():
    logger.log("Now listening at: 127.0.0.1:" + str(listen_PORT))
    server.listen()
